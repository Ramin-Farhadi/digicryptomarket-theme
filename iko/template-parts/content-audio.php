...
$iko_audio_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
...
<?php if (is_single()) : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-standard-post blog-details-wrap format-audio'); ?>>

        <?php if (!empty($iko_audio_url)) : ?>
            <div class="blog-standard-thumb postbox__audio ratio ratio-16x9">
                <?php echo wp_oembed_get($iko_audio_url); ?>
            </div>
        <?php endif; ?>
...
<?php else : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-standard-post format-audio'); ?>>
