...
function iko_widgets_init() {
     ...
     */
    register_sidebar( [
        'name'          => esc_html__( 'Blog Sidebar', 'iko' ),
        'id'            => 'blog-sidebar',
    ...
    for ( $num = 1; $num <= $footer_widgets; $num++ ) {
        register_sidebar( [
            'name'          => sprintf( esc_html__( 'Footer widget no. %1$s', 'iko' ), $num ),
            'id'            => 'footer-' . $num,
            'description'   => sprintf( esc_html__( 'Footer Column %1$s', 'iko' ), $num ),
...
add_action( 'widgets_init', 'iko_widgets_init' );